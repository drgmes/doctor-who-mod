...
public class TardisConsoleUnitToyotaBlockRenderer extends BaseTardisConsoleUnitBlockRenderer<TardisConsoleUnitToyotaBlockEntity> {
    public TardisConsoleUnitToyotaBlockRenderer(BlockEntityRendererFactory.Context context) {
        super(context, TardisConsoleUnitTypes.TOYOTA);
    }

    @Override
    public void render(TardisConsoleUnitToyotaBlockEntity tile, float delta, MatrixStack matrixStack, VertexConsumerProvider buffer, int light, int overlay) {
        ModelPart modelRoot = this.ctx.getLayerModelPart(TardisConsoleUnitToyotaModel.LAYER_LOCATION);
        TardisConsoleUnitToyotaModel model = new TardisConsoleUnitToyotaModel(modelRoot);
        VertexConsumer vertexConsumer = buffer.getBuffer(model.getLayer(TardisConsoleUnitToyotaModel.LAYER_LOCATION.getId()));
    ...
    private void renderSonicScrewdriver(TardisConsoleUnitToyotaBlockEntity tile, MatrixStack matrixStack, VertexConsumerProvider buffer, int light, int overlay, float rotateDegrees) {
